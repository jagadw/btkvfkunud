<div class="d-flex flex-column flex-column-fluid">
    <x-slot:title>Verifikasi Tindakan</x-slot:title>

    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-1 flex-column justify-content-center my-0">Verifikasi Tinndakan</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-5  my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">Verifikasi Tindakan</li>
                </ul>
            </div>
                ...
                <div class="row mb-3 align-items-end g-2">
                    <div class="col-md-4">
                        ...
                        <div class="position-relative">
                            ...
                            <input type="text" class="form-control form-control-solid ps-10 border-primary border-3 text-primary"
                                placeholder="Cari Nama / No Rekam Medis" wire:model.live.debounce.100ms="search" />
                        </div>
                    </div>
                    <div class="col-md-5">
                        <label class="mb-1">Rentang Waktu Operasi</label>
                        <div class="d-flex gap-2">
                            <input type="date" id="tanggal_operasi_start" class="form-control" wire:model="tanggal_operasi_start">
                            <span class="align-self-center">s/d</span>
                            <input type="date" id="tanggal_operasi_end" class="form-control" wire:model="tanggal_operasi_end">
                        </div>
                    </div>
                    <div class="col-md-3 d-flex align-items-end gap-2">
                        <button class="btn btn-success fw-bold" wire:click="verifikasi"
                            @if(empty($selectedTindakans)) disabled @endif>
                    ...
                    <table id="table-responsive" class="table table-row-bordered table-striped gy-5">
                        <thead>
                            <tr class="fw-semibold fs-6 border-2 text-center align-middle">
                                <th rowspan="2" class="align-middle text-center" style="width: 50px;">
                                    <div class="form-check d-flex justify-content-center align-items-center">
                                        <input
                                            ...
                                            @if(count($selectedTindakans)===$tindakans->count() && $tindakans->count() > 0) checked @endif
                                        />
                                    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="text-center">
                                    <div class="form-check d-flex justify-content-center align-items-center">
                                        <input
                                            ...
                                            type="checkbox"
                                            value="{{ $t->id }}"
                                            wire:model="selectedTindakans" />
                                    </div>
                                </td>
                                <td class="text-center">
                                    @if ($t->verifikasi == 0)
                                    <button class="btn btn-sm btn-success d-flex align-items-center" wire:click="verifikasiSingle({{ $t->id }})">
                                        <i class="bi bi-check2-circle"></i>
                                        Verifikasi
                                    </button>
                                    @else
                                ...
                                @for ($i = $asistens->count(); $i < $maxAsisten; $i++)
                                    ...
                                    <td class="text-center align-items-center">{{ $t->conference?->realisasi_tindakan ?? '-' }}</td>
                                    <td class="text-center align-items-center">
                                        @if ($t->foto_tindakan)
                                    ...
                                    <td class="text-center align-items-center">
                                        @if ($t->verifikasi == 1)
                                        <span class="badge text-white bg-success">Sudah Diverifikasi</span>
                                        @else
                                        <span class="badge text-white bg-danger">Belum Diverifikasi</span>
                                        @endif
                                    </td>
                            </tr>
                            @empty
